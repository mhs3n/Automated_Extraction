...
- **LLM-Based Information Extraction:**# Cahier des Charges: Automated Information Extraction System
...
- **Performance:** Process documents efficiently (target <3 seconds per page).
...
- **Logging Mechanism:**
  ...
  - Store logs in a centralized location for easy debugging and auditing.
  - Use tools like Loguru or Python’s built-in logging module.
- **Performance Metrics:**
  - Track document processing time (OCR + LLM inference).
  - Monitor system resource usage (CPU, RAM, GPU if applicable).
  - Implement real-time monitoring using Prometheus & Grafana.
